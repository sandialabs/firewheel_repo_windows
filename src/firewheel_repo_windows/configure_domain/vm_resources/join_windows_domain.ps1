...
[Parameter(Mandatory=$False)][string]$dns_ip
)
...
try {
    ...
    if($PSBoundParameters.ContainsKey('dns_ip')) {
        Get-WmiObject win32_networkadapterconfiguration -filter "ipenabled = 'true'" | ForEach-Object { $_.SetDNSServerSearchOrder($dns_ip) }
    } else {
    ...
    Get-WmiObject win32_networkadapterconfiguration -filter "ipenabled = 'true'" | ForEach-Object { 
        $_.SetWINSServer($domain_controller_ip, $domain_controller_ip)
    }
